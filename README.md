...
- **Joi (Input Validation)**: Security is critical for e-commerce. `Joi` is used as a strict schema-based validation layer to sanitize and verify all incoming requests (Authentication, Cart modifications, Order placement, Contact forms) before they reach the controllers, preventing malformed data or injection attempts.
...
- **Redis (Caching & Rate Limiting)**: E-commerce platforms are extremely read-heavy. Redis is utilized to cache frequent MongoDB queries (like fetching the product catalog), drastically reducing database load and delivering lightning-fast product pages to the frontend.
  - **Distributed Rate Limiting**: Redis also powers a robust rate-limiting system across the entire API to prevent abuse and DDoS attacks. A `globalLimiter` restricts general API access (100 requests / 15 mins), while a stricter `authLimiter` protects authentication routes (10 requests / 15 mins) from brute-force attacks.
